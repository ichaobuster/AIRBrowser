package com.ichaobuster.browser.component.basic
{
	...
	import com.ichaobuster.browser.component.basic.CustomHTMLHost;
	 ...
	 *  @productversion Flex 3
	 */
	public class CustomHTML extends ScrollControlBase
...
//		include "../core/Version.as";
		 ...
		 */
		private var _htmlHost:HTMLHost = new CustomHTMLHost();
		...
		override protected function createChildren():void
		{
			...
			if (!htmlLoader)
			{
				...
				htmlLoader.addEventListener(
					...
					htmlLoader_locationChangingHandler);
...
//				label = "正在加载...";
		...
		{
			dispatchEvent(event);
			// dispatchPropertyChangeEvent("label", label, htmlLoader.window.document.title);
			label = htmlLoader.window.document.title;
		}
		...
		protected function htmlLoader_locationChangingHandler(event:LocationChangeEvent):void
		{
			event.preventDefault();
			htmlLoader.cancelLoad();
		...
		[Event(name="startDownload", type="flash.events.Event")]
		[Event(name="completeDownload", type="flash.events.Event")]
		private function updateLocation(locationURL:String):void
		{
			var urlRequest:URLRequest = new URLRequest(locationURL);
			...
			urlLoader.addEventListener(HTTPStatusEvent.HTTP_RESPONSE_STATUS,function(event:HTTPStatusEvent):void{
				...
				{
					if (rspHeader.name.toLowerCase() == "content-type")
					{
						// 普通HTML页面
						if (rspHeader.value.toLowerCase().indexOf("text/html")!=-1)
						{
							break ;
						}else // 下载请求
						{
							var startDownloadEvent:Event = new Event("startDownload");
							dispatchEvent(startDownloadEvent);
							filename = httpResponseUtils.getFileNameFromResponseHeader(responseHeaders, locationURL);
						}
						event.preventDefault();
						return ;
					}
				}
				var htmlRequest:URLRequest = new URLRequest(locationURL);
			...
			urlLoader.addEventListener(Event.COMPLETE, function(event:Event):void{
				var completeDownloadEvent:Event = new Event("completeDownload");
				dispatchEvent(completeDownloadEvent);
				var bytesLength:int = urlLoader.bytesAvailable;
				var dataArr:ByteArray = new ByteArray();
				urlLoader.readBytes(dataArr,0,bytesLength);
				
//				var downFile:File= new File(File.documentsDirectory.nativePath);
//				trace(downFile.nativePath);
//				downFile.save(dateArr, filename);
				
				var fileDownloadEvent:FileDownloadEvent = new FileDownloadEvent(FileDownloadEvent.FILE_DOWNLOAD_EVENT, dataArr, filename);
				dispatchEvent(fileDownloadEvent);
			});
			
			urlLoader.addEventListener(IOErrorEvent.IO_ERROR, function(event:IOErrorEvent):void
			{
				var htmlRequest:URLRequest = new URLRequest("assets/page/NetworkError.htm");
				htmlLoader.load(htmlRequest);
			});
		...
		public function loadBlankPage():void{
			var htmlRequest:URLRequest = new URLRequest("assets/page/about_blank.html");
			htmlLoader.load(htmlRequest);
		}
	}
